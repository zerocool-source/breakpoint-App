...
export function registerTechOpsRoutes(app: Express) {
  ...
  // Pool Brain API Sync: DISABLED - only using local/mobile app data
  app.post("/api/tech-ops/sync-repairs", async (_req: Request, res: Response) => {
    // Pool Brain sync disabled - return empty result
    res.json({ synced: 0, repairEntries: [], windyDayEntries: [], message: "Pool Brain sync disabled - using mobile app data only" });
  });
