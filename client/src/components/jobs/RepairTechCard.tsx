...
export function RepairTechCard({ tech, monthlyQuota }: { tech: RepairTechData; monthlyQuota: number }) {
  ...
  return (
    <Collapsible open={isExpanded} onOpenChange={setIsExpanded}>
      <Card className="bg-gradient-to-br from-slate-800/90 to-slate-900/80 border-[#0078D4]/40 hover:border-[#0078D4]/60 hover:shadow-[0_0_20px_rgba(56,189,248,0.15)] transition-all duration-300 shadow-lg" data-testid={`repair-tech-${tech.name}`}>
            ...
            <div className="bg-slate-700/50 rounded-lg p-3 border border-slate-600/50 mt-3">
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="text-sm font-ui">
                  <span className={tech.quotaPercent >= 100 ? "text-[#0078D4] font-bold" : "text-slate-200"}>
              ...
              <div className="flex justify-between mt-1">
                <span className={`text-xs font-semibold ${tech.quotaPercent >= 100 ? 'text-[#0078D4]' : 'text-slate-300'}`}>
                  {tech.quotaPercent}%
                </span>
                  ...
                  return (
                    <div
                      ...
                      className={`w-4 h-4 rounded-sm flex items-center justify-center text-[8px] font-bold transition-colors ${
                        hasActivity 
                          ? intensity > 0.7 ? 'bg-[#0078D4] text-white' 
                          : intensity > 0.3 ? 'bg-[#0078D4]/60 text-white' 
                          : 'bg-[#0078D4]/30 text-[#0078D4]'
