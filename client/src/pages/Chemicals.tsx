...
import { Droplets, MapPin, Building2, Phone, Mail, User, ChevronDown, AlertCircle, RefreshCw, Clock, CheckCircle2 } from "lucide-react";
...
export default function Chemicals() {
  ...
  const resolvedChemicals = chemicalAlerts.filter(a => a.status !== "Active");

  const displayedAlerts = showResolved ? chemicalAlerts : activeChemicals;
  ...
  const urgentCount = activeChemicals.filter(a => a.severity.toUpperCase() === "URGENT").length;
  ...
  return (
    <AppLayout>
      <div className="mb-8 flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground font-ui tracking-wide">
            {activeChemicals.length} Active Chemical Alerts • {urgentCount > 0 ? `${urgentCount} Urgent` : "No urgent alerts"}
          </p>
        </div>
        <Button
          onClick={() => refetch()}
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
        <Card className="glass-card border-white/5">
          <CardContent className="p-4 flex items-center gap-4">
            ...
            <div>
              <p className="text-2xl font-bold text-white font-ui">{activeChemicals.length}</p>
          ...
          <CardContent className="p-4 flex items-center gap-4">
            ...
            <div>
              <p className="text-2xl font-bold text-white font-ui">
                {activeChemicals.filter(a => a.severity.toUpperCase() === "HIGH").length}
              </p>
        ...
        <Card className="glass-card border-white/5">
          <CardContent className="p-4 flex items-center gap-4">
            ...
            <div>
              <p className="text-2xl font-bold text-white font-ui">{resolvedChemicals.length}</p>
      ...
      <div className="flex gap-2 mb-6">
        ...
        >
          All Chemical Alerts ({chemicalAlerts.length})
        </Button>
      </div>
      ...
      <Card className="glass-card border-white/5">
        <CardHeader className="pb-2">
          <CardTitle className="font-display text-lg tracking-wide flex items-center gap-2">
            ...
            {showResolved ? "ALL CHEMICAL ALERTS" : "ACTIVE CHEMICAL ALERTS"}
          </CardTitle>
        </CardHeader>
        <CardContent>
          {isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center h-40 text-muted-foreground">
              ...
              <p>No {showResolved ? "" : "active "}chemical alerts found</p>
              <p className="text-xs mt-1">All chemical levels are normal</p>
            </div>
          ) : (
